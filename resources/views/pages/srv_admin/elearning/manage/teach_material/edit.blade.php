                    ...
                    <div class="kt-portlet__body kt-wizard-v4__content">
                        <div class="kt-section kt-section--first">
                            <div class="kt-section__body">
                                ...
                                <input type="hidden" name="id_teach_material" value="{{ $teachM->id_tech_material }}">
                                <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <input value="{{ $teachM->title }}" name="title" class="form-control">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea type="text" name="description" id="editor1">{{ $teachM->description }}</textarea>
                                    ...
                                    <div class="col-xl-8">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input value="{{ $teachM->main_file_path }}" id="real_path" type="hidden" name="main_file_path">
                                                <div id="err_main_file_path" class="invalid-feedback" style="display:block; font-size:14px">
                                                </div>
                                            </div>
                                            <span class="form-text text-muted">@lang('courses.teach-create-f-3-sub-up')</span>
                                        </div>
                                    </div>
                                    <div class="col-xl-4 kt-hidden">
                                        <div class="form-group" style="display: table-cell; vertical-align: middle; padding-bottom: 15px">
                                            <?php
                                            // $is_allow_download = ($teachM->allow_download == 1) ? "disabled" : "";

                                            $extension = '';
                                            if ($teachM->main_file_path != null) {
                                                $cleanUrl = str_replace('http://elcat.localhost/', '', $teachM->main_file_path);
                                                $localingPath = str_replace('/', '\\', $cleanUrl);
                                                $final = public_path($localingPath);
                                                $extension = pathinfo($final)['extension'];
                                            }
                                            $valid_ex = array('pdf', 'mp3', 'mp4', 'ogg', 'jpg', 'png');

                                            $class_disabled = 'kt-checkbox--disabled';
                                            $is_disabled = 'disabled';
                                            if (in_array($extension, $valid_ex)) {
                                                $class_disabled = '';
                                                $is_disabled = '';
                                            }
                                            ?>
                                            <label id="lb_allow_download" class="kt-checkbox kt-checkbox--bold kt-checkbox--brand {{ $class_disabled }}">
                                                <input type="checkbox" id="allow_download" name="allow_download" value="1" {{ $is_disabled }} {{ ($teachM->allow_download == 1) ? 'checked':'' }}> @lang('courses.teach-create-f-4')
                                                <span></span>
                                            </label>
                                        </div>
                                    </div>
...
@if($teachM->main_file_path != null)
    <?php
        $explo = explode('/',$teachM->main_file_path);
        $fileName = end($explo);
    ?>
    <script>
        $(document).ready(function(){
            var file_name = "{{ $fileName }}";
            $('#thumbnail').val('').val(file_name).trigger('change');
        });
...
<script>
    ...
    btn.on('click', function(e) {
        ...
        var update_url = '{{ route("srv_admin.courses.teach.update",":uuid") }}';
            ...
            success: function(response) {
                ...
                if ($.isEmptyObject(response.error)) {
                    ...
                    swal.fire({
                        ...
                        confirmButtonText: "@lang('general.msg-success-ok-btn')",
                    }).then((result) => {
                        if (result.value) {
