            ...
            <div class="kt-portlet" id="form-wrapper">
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">@lang('participant.title') <small>@lang('participant.sub-title')</small></h3>
                    </div>
                    ...
                    <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('users.tb-h-7')</th>
                                <th>@lang('users.tb-h-8')</th>
            ...
            <form id="kt_form" class="kt-form kt-form--fit kt-form--label-right">
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="invalid-feedback" id="err_selected_users" style="display:block; font-size:14px"></div>
                    </div>
...
<script>
    ...
    function participantSelect2Load() {
        ...
        users_url = users_url.replace(":uuid", "{{ $courses->uuid }}");
...
<script>
    ...
    btn.on('click', function(e) {
        ...
        var enroll_url = '{{ route("srv_admin.courses.enroll",":uuid") }}';
        enroll_url = enroll_url.replace(":uuid", "{{ $courses->uuid }}");
            ...
            success: function(response) {
                ...
                if ($.isEmptyObject(response.error)) {
                    ...
                    for (let [key, value] of Object.entries(response.error)) {
                        ...
                        $('#err_' + key).text(value.toString().replace(key.toString().replace('_', ' '), 'Bidang '));
...
<script>
    ...
    $(document).ready(function() {
        table = $('#kt_table_1').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route("srv_admin.courses.participants.to.json","$courses->uuid") }}',
            columns: [{
                    data: 'name',
                    name: 'users.name'
                },
                ...
                // },
                {
                    "mRender": function(data, type, full, meta) {
                        var str_action = `
                            <a href="{{ route('srv_admin.courses.participant.view',[':uuid',':id_user']) }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Lihat">
                        ...
                        if(full['id'] != '{{ auth()->id() }}'){
                            str_action += `<a href="#" onclick="deleteParticipant(':uuid',':id_user');" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Delete">
                            <i class="la la-trash"></i>
                            </a>;`
                        }

                        str_action = str_action
                                    .replace(":id_user", full['id'])
                                    .replace(":id_user", full['id'])
                                    .replace(":uuid", "{{ $courses->uuid }}")
                                    .replace(":uuid", "{{ $courses->uuid }}");
...
<script>
    function deleteParticipant(uuid, id_user) {
        swal.fire({
            "title": "@lang('courses.prt-title-del')",
            "html": "@lang('courses.prt-content-del')",
            "type": "warning",
            "confirmButtonClass": "btn btn-primary",
            "cancelButtonClass": "btn btn-secondary",
            confirmButtonText: "@lang('courses.prt-btn-yes')",
            cancelButtonText: "@lang('courses.prt-btn-no')",
            showCancelButton: true,
        }).then((result) => {
            if (result.value) {
                var unenroll_url = '{{ route("srv_admin.courses.delete",":uuid") }}';
                unenroll_url = unenroll_url.replace(":uuid", "{{ $courses->uuid }}");
                $.ajax({
                    url: unenroll_url,
                    method: 'post',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        swal.fire({
                            ...
                            "html": "Partisipan berhasil dikeluarkan dari kursus",
                            "type": "success",
                            "confirmButtonClass": "btn btn-primary",
                            cancelButtonClass: "btn btn-secondary",
                            showCancelButton: false,
